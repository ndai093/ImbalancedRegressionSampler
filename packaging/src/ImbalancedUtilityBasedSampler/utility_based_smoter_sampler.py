...
class UtilityBasedSmoteRSampler:
    """
    Class UtilityBasedSmoteRSampler takes arguments as follows:
    ...
    def __init__(self, data, method='auto', extrType='both', thr_rel=1.0, controlPts=[], c_perc="balance", k=5):
        ...
        if str == type(c_perc):
            ...
            self.__processCPerc(c_perc)
        
        self.k = k
        self.coef = 1.5

    def __processCPerc(self, c_perc):
        for x in c_perc:
            if x < 1.0:
    ...
    def resample(self):
        ...
        #interesting set
        self.__set_obj_interesting_set(data1)
        #uninteresting set
        self.__set_obj_uninteresting_set(data1)
        #calculate bumps
        self.__set_obj_bumps(data1)

        if self.c_perc == 'percentage list':
            resampled = self.__process_percentage()
        elif self.c_perc == 'balance':
            resampled = self.__process_balance()
        elif self.c_perc == 'extreme':
            resampled = self.__process_extreme()
    ...
    def __process_percentage(self):
        undersampling_and_interesting, new_samples_set = self.__preprocess_percentage()
        ...
        for index, value in dups_sample_counts.items():
            ...
            kNN_result = self.__kNN_calc(self.k, base_sample, interesting_set_list)
    ...
    def __preprocess_percentage(self):
    ...
    def __kNN_calc(self, k, sample_as_list, interesting_set_list):
    ...
    def __process_balance(self):
        new_samples_set = self.__preprocess_balance()
        reduced_cols = new_samples_set.columns.values.tolist()[:-1]
        dups_sample_counts = new_samples_set.pivot_table(index=reduced_cols, aggfunc='size')
        interesting_set_list = self.interesting_set.iloc[:,:-1].values.tolist()
    ...
    def __process_extreme(self):
        new_samples_set = self.__preprocess_extreme()
        reduced_cols = new_samples_set.columns.values.tolist()[:-1]
        dups_sample_counts = new_samples_set.pivot_table(index=reduced_cols, aggfunc='size')
        interesting_set_list = self.interesting_set.iloc[:,:-1].values.tolist()
