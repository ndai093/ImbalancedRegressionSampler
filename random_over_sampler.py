...
class RandomOverSamplerRegression:
    """
    Class RandomOverSamplerRegression takes arguments as follows:
        ...
        c_perc - oversampling strategy should be applied in each bump with interesting sets, 
                 ...
                 "<percentage>" - percentage oversampling interesting bumps should follow.
    """
    ...
    def resample(self):
        ...
        #uninteresting set
        bumps_uninteresting, bumps_oversampling = self.calc_bumps(data1)

        if self.c_perc == 'balance':
            resampled = self.process_balance(bumps_oversampling, uninteresting_set)
        elif self.c_perc == 'extreme':
            resampled = self.process_extreme(bumps_oversampling, bumps_uninteresting, uninteresting_set)
        elif isinstance(self.c_perc, list):
            resampled = self.process_percentage(bumps_oversampling, uninteresting_set)
    ...
    def process_balance(self, bumps_oversampling, uninteresting_set):
    ...
    def process_extreme(self, bumps_oversampling, bumps_uninteresting, uninteresting_set):
    ...
    def process_percentage(self, bumps_oversampling, uninteresting_set):
